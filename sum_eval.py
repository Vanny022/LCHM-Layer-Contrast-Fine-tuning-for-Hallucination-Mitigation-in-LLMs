...
def load_csv(file_path):
    ...
    # --- A) QA 四列 ---
    if {"document","right_summary","hallucinated_summary"} <= cols:
        for idx in range(len(df)):
            item = dict(
                prefix=str(df['document'][idx]),
                completion=str(df['right_summary'][idx]),
                contradiction_0=str(df['hallucinated_summary'][idx]),
                contradiction_1=str(df['hallucinated_summary'][idx]),
                contradiction_2=str(df['hallucinated_summary'][idx]),
            )
