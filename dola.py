        ...
        with torch.no_grad():
            ...
            if mode == 'baseline':
                ...
                for idx, layer_id in enumerate(candidate_premature_layers):
                    print(f"Layer {layer_id}: JS Divergence = {js_divs[idx].item():.6f}")

                # 输出差异最大的 premature layer
                max_layer = candidate_premature_layers[int(js_divs.argmax().cpu().item())]
                print(f"Most different layer: {max_layer}")
