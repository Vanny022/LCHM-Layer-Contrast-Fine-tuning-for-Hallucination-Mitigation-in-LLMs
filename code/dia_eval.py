...
def load_csv(file_path):
    ...
    # --- A) QA 四列 ---
    if {"knowledge","dialogue_history","right_response","hallucinated_response"} <= cols:
        for idx in range(len(df)):
            prefix = (
            f"{str(df['knowledge'][idx]).strip()}\n\n"
                f"Q: {str(df['dialogue_history'][idx]).strip()}\nA: ")
            item = dict(
                prefix=prefix,
                completion=str(df['right_response'][idx]),
                contradiction_0=str(df['hallucinated_response'][idx]),
                contradiction_1=str(df['hallucinated_response'][idx]),
                contradiction_2=str(df['hallucinated_response'][idx]),
            )
