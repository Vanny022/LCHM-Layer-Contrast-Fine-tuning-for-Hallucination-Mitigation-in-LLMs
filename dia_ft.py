...
def load_frame(path: str) -> pd.DataFrame:
    ...
    # 如果是四列 QA 数据
    if {"knowledge","dialogue_history","right_response","hallucinated_response"} <= set(df.columns):
        df["prompt_text"] = (
        df["knowledge"].fillna("").astype(str) 
        + "\n\nQ: " 
        + df["dialogue_history"].fillna("").astype(str) 
        + "\nA:"
         ).str.strip()
    
        df["answer_text"] = df["right_response"].fillna("").astype(str)
...
def build_example(tokenizer, prompt: str, answer: str, max_len: int):
    ...
    max_a = min(len(a_ids), max_len - 1)        
    max_p = max_len - 1 - max_a
    p_ids = p_ids[-max(0, max_p):]
    a_ids = a_ids[:max_a]
