...
def load_frame(path: str) -> pd.DataFrame:
    ...
    # 如果是四列 QA 数据
    if {"user_query", "chatgpt_response", "hallucination", "oppositeanswer"} <= set(df.columns):
        df["prompt_text"] = (
        df["user_query"].fillna("").astype(str) 
        + "\n\nQ: " 
        + df["chatgpt_response"].fillna("").astype(str) 
        + "\nA:"
         ).str.strip()
        df["answer_text"] = df["hallucination"].fillna("").astype(str)
