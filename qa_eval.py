...
def load_csv(file_path):
    """
    支持两种格式：
    1) QA 四列 CSV: knowledge, question, right_answer, hallucinated_answer
       -> 转换为 {prefix, completion, contradiction_0/1/2}
    2) 原 FACTOR CSV: full_prefix/turncated_prefixes, completion, contradiction_0/1/2
       -> 保持原样
    """
    import pandas as pd
    import os
    ...
    # --- A) QA 四列 ---
    if {"knowledge", "question", "right_answer", "hallucinated_answer"} <= cols:
        for idx in range(len(df)):
            prefix = f"{str(df['knowledge'][idx]).strip()}\n\nQ: {str(df['question'][idx]).strip()}\nA: "
            item = dict(
                prefix=prefix,
                completion=str(df['right_answer'][idx]),
                contradiction_0=str(df['hallucinated_answer'][idx]),
                contradiction_1=str(df['hallucinated_answer'][idx]),
                contradiction_2=str(df['hallucinated_answer'][idx]),
            )
    ...
    need_cols = {prefix_type, 'completion', 'contradiction_0', 'contradiction_1', 'contradiction_2'}
    if not need_cols.issubset(cols):
        raise ValueError(f"无法识别 CSV 格式，列名: {cols}")
...
if __name__ == "__main__":
    ...
    if args.debug:
        list_data_dict = list_data_dict[:100000000000]
