...
def load_csv(file_path):
    ...
    # --- A) QA 四列 ---
    if {"user_query", "chatgpt_response", "hallucination", "oppositeanswer"} <= cols:
        for idx in range(len(df)):
            prefix = f"{str(df['user_query'][idx]).strip()}\n\nQ: {str(df['chatgpt_response'][idx]).strip()}\nA: "
            item = dict(
                prefix=prefix,
                completion=str(df['hallucination'][idx]),
                contradiction_0=str(df['oppositeanswer'][idx]),
                contradiction_1=str(df['oppositeanswer'][idx]),
                contradiction_2=str(df['oppositeanswer'][idx]),
            )
